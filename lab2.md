...
- The main method takes the port number as an argument, and an int called port is initialized with this value. handleRequest takes a URI called url (localhost:4000/add-message?s=Hello) as an argument, and a List of strings called lines is initialized to be empty. A string called query is initialized with the query part of url (part after "?" which is "Hello"). Lastly, an integer called count is initialized to 0.   
...
- The main method uses the same port number as an argument, and handleRequest takes the URI url (localhost:4000/add-message?s=How%20are%20you) as an argument. query is initialized with the query part of url (How%20are%20you) where %20 is a space
